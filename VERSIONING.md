...
gemchain guard ask-honeybadger
gemchain update ask-honeybadger <new-version> --dry-run
gemchain update ask-honeybadger <new-version>
```
...
3. Build passes — `bundle exec rake build` (or
   `gem build ask-honeybadger.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/honeybadger/version.rb` by gemchain, not by
   hand.
...
| `lib/ask/honeybadger/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Several bug fixes ship together: one release, `0.1.5` → `0.1.6` (never
  `0.1.8`).
- New backwards-compatible API: `0.1.6` → `0.2.0`.
